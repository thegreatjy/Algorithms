...
public class Solution {
	...
	public int solution(int bridge_length, int weight, int[] truck_weights) {
        int days = 0;
        int idx=0;
        int weightSum=0;	//다리에 있는 트럭의 무게
        Queue<Integer> bridge = new LinkedList<Integer>();
         
        while(true) {
        	//트럭을 다 옮긴 경우
        	if(idx>=truck_weights.length) {
        		//다리 위에 있는 트럭을 다 옮기고 끝냄
        	...
        	//현재 트럭을 다리에 넣을 수 있는 경우 
        	if((bridge.size()+1<=bridge_length) && (weightSum+truck_weights[idx]<=weight)) {
        		bridge.add(truck_weights[idx]);
        		weightSum+=truck_weights[idx];
        		days++;
        		idx++;
        	}else {//현재트럭을 다리에 넣을 수 없음 
        		//다리에 있는 트럭을 꺼냄 
        		while(!bridge.isEmpty()) {
        			int temp=bridge.poll();
        			weightSum=weightSum-temp;
	...
	public static void main(String[] args) {
		...
		result=new Solution().solution2(bridge_length, weight, truck_weights);
		System.out.println(result);
	}
